...
namespace DtsTest
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("\n=== 生成DTS文件 ===");
            var generator2 = new DeviceTreeGenerator();
            var generatedContent2 = generator2.Generate(deviceTree);
            File.WriteAllText("generate.dts", generatedContent2);
            ...
            // 添加新节点
            var newNode = editor.AddNode("/", "my-device", 1000000);
            ...
            var generatedContent = generator.Generate(deviceTree);

            File.WriteAllText("output.dts", generatedContent);
            Console.WriteLine("已生成 output.dts 文件");
            ...
            // 复制节点
            var copiedNode = editor.CopyNode("/my-device@1000000", "/", "my-device-copy@2000000");
            if (copiedNode != null)
            {
                editor.UpdateProperty("/my-device-copy@2000000", "reg", new int[] { 0x2000000, 0x1000 });
                Console.WriteLine("复制并修改了设备节点");
            }

            // 查找和替换
            int replacedCount = editor.FindAndReplaceProperty("status", "okay", "disabled");
            Console.WriteLine($"替换了 {replacedCount} 个status属性");
